...
# Removed title to save space as requested
# st.title("Coinmap API Explorer")
...
# --- Mode Selection ---
# Moved title to header alongside mode selection for compact view if needed, 
# or just removed main title space.
st.header("Coinmap API Explorer") # Use header instead of title for less padding
...
if mode == "Raw API Explorer":
    # ==========================================
    # MODE A: Raw API Explorer (Existing)
    # ==========================================
    ...
    if exchange_mode.startswith("OKX"):
        scenarios = {
            "1. Current Balance (현재 자산)": {
                ...
                "params": {"ccy": "USDT"}, # Initial suggestion, but we will ignore this to get ALL balances
                "logic_type": "balance"
            },
            "2. Positions (포지션 현황)": {
                "exchange": "OKX",
        ...
        if scenario['exchange'] == "OKX":
            ...
            if scenario['logic_type'] == 'balance':
             ...
             if scenario['logic_type'] in ['binance_balance', 'binance_positions']:
                 # For balance/positions, we use the full response object which is a dict
                 # We'll process it specifically in the logic block
                 # Just pass the raw dict wrapped in a list for now or handle later?
                 # Let's keep df empty and use raw_res directly for these two
                 pass 
        ...
        if logic_type == "balance":
             if df.empty:
                 ...
                 if not usdt_row.empty:
                     ...
                     # Note: Usually (Balance Used for Margin) = Total Balance - Available Balance - UPL (roughly)
                     # But spec asks for: eq - availBal - upl
            ...
            # For Instruments, we need to query per instrument or list.
            # Since we don't know which instruments user has, best is to fetch all SWAP instruments.
            # BUT this can be heavy.
            # Alternative: First get positions, then fetch instruments for specific instIds.
            ...
            **적용된 로직:**
            ...
            # Updated Logic based on test_binance_4_flow.py
            # We need to explicitly pass incomeType="TRANSFER" in params to get correct data
            # instead of filtering it client-side from a generic call which might miss it due to limits.
            ...
            # Force params to include incomeType=TRANSFER
            # This overrides what was sent in send_request if we did it manually here, 
            # but send_request used 'params' from the scenario definition.
            # We need to make sure the scenario definition includes this or we modify it here.
            # Since we already fetched 'df' from the generic call, let's check if we need to re-fetch.
            
            # The issue: Spec says filter incomeType='TRANSFER', but if the user has many trades,
            # a generic /income call with limit=1000 might be filled with 'COMMISSION' or 'REALIZED_PNL'
            # pushing 'TRANSFER' out of the list.
            # Solution: Re-fetch with specific param.
            ...
            with st.spinner("Re-fetching with incomeType='TRANSFER'..."):
                ...
                # If start_ts_ms is valid, include it to filter server-side if possible
                # Binance /income supports 'startTime' and 'endTime'
                # However, our unified logic uses client-side filtering.
                # To be safe and get MORE history, let's add startTime to params if it's reasonable.
                # Spec doesn't strictly require it but it helps if the user has >1000 transfers since account creation.
                if start_ts_ms > 0:
    ...
    def update_end_bal_from_trades():
        ...
        # Direct assignment to session state key associated with a widget is not allowed 
        # if the callback is triggered by another widget, SOMETIMES. 
        # Streamlit says: "st.session_state.sim_end_bal cannot be modified after the widget with key sim_end_bal is instantiated."
        # This happens because update_end_bal_from_trades is called inside the button click block AFTER the widget for sim_end_bal is already rendered (instantiated).
        # In Streamlit, you can only update session state for a widget BEFORE the widget is rendered in that rerun, OR from a callback attached to that widget (or another widget if it triggers a rerun immediately).
        
        # Fix: We perform the calculation and store it. But updating the KEY 'sim_end_bal' after it was rendered at line 961 causes the error.
        # Line 961: val_end_bal = st.number_input(..., key="sim_end_bal")
        # The button click handler (lines 992+) runs AFTER line 961.
        
        # Solution: Do not call this function inside the button block if it modifies 'sim_end_bal'. 
        # Instead, modify 'sim_trades' in the button block, and let the script RERUN. 
        # Upon Rerun, we should calculate the new balance BEFORE line 961.
        
        # We will move this calculation logic to top of the section, before widgets are rendered.
        pass

    # Logic to update balance (Auto-Calc) runs at START of this section
    # We need to calculate `sim_end_bal` based on inputs.
    # But inputs (Start Bal, etc.) are in widgets. We get them from session state.
    
    # Re-structure:
    # 1. Handle Button Clicks (Add/Remove Trade) -> Update State -> Rerun
    # 2. Calculate End Balance based on current State
    # 3. Render Widgets (End Balance Widget gets value from State)
    
    # --- Handle Button Actions (Before Widgets) ---
    # But buttons are inside columns layout... Streamlit buttons return True only when clicked.
    # If we place buttons after widgets, we can't update widget state.
    # Streamlit pattern: Use callbacks on buttons? 
    # st.button(..., on_click=add_trade_callback)
    ...
    # Row 1.5: Manual PnL Input (List Interface)
    # st.number_input("선물 손익 (가정)", value=0.0, key="sim_manual_pnl", on_change=recalc_balance, help="선물 거래로 인한 순수 손익 (입출금/현물거래 제외)")
    
    st.markdown("**선물 손익 (가정)**")
    ...
    with c5:
        ...
        if exchange_mode.startswith("OKX"):
            ...
            st.number_input("현재 현물 잔고 (추정)", value=val_current_spot, disabled=True, help="초기 + 매수 - (매도 - PnL). 순차적 계산 결과.")
            
            st.button("Reset Trades", on_click=reset_trades_callback)
        else:
